...
using Eigen::Dynamic, Eigen::NoChange, Eigen::all, Eigen::MatrixBase;

template <typename T> using Vec = Eigen::Vector<T, Dynamic>;

template <typename T> using Mat = Eigen::Matrix<T, Dynamic, Dynamic>;

using cmb::constrained_miniball, cmb::RealMatrix, cmb::RealVector;
...
int main() {
	...
	cout << "Using Eigen tolerance : " << Eigen::NumTraits<S>::dummy_precision()
		 << endl;
	...
	cout << "Delta centre (squared norm) : " << (centre - correct_centre).norm()
		 << endl;
