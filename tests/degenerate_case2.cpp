...
auto main(int /*argc*/, char* /*argv*/[]) -> int {
	...
	CGAL::print(cerr, sq_radius.numerator());
	cerr << '/';
	CGAL::print(cerr, sq_radius.denominator());
	cerr << " (exact), " << CGAL::to_double(sq_radius) << " (double)" << '\n';
	...
	CGAL::print(cerr, sq_radius1.numerator());
	cerr << '/';
	CGAL::print(cerr, sq_radius1.denominator());
	cerr << " (exact), " << CGAL::to_double(sq_radius1) << " (double)" << '\n';
